...
# 在开发机（已装这些包）执行：
$QMT_SITE = "D:\guojinqmt\bin.x64\Lib\site-packages"
...
把以下内容复制到大 QMT 的 `python` 目录（如 `D:\guojinqmt\python\`）：
...
def _known_qmt_python_dir():
    ...
    # 解码后 = D:\guojinqmt\python
```

- **`chr()` 编码**（旧实现）是为了规避 QMT 用 GBK 保存策略文件时中文乱码；现默认安装目录为纯 ASCII（`D:\guojinqmt\python`），已无需该技巧。
